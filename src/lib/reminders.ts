...
export function generateReminders(data: AppData): Reminder[] {
  ...
  // 7. A wish you can now actually afford, based on this month's safe-to-spend.
  if (data.wishes.length > 0) {
    const affordable = data.wishes.filter((w) => w.price > 0 && w.price <= safeToSpend);
    if (affordable.length > 0) {
      const cheapest = affordable.reduce((min, w) => (w.price < min.price ? w : min));
      reminders.push({
        id: `wish_affordable_${cheapest.id}_${monthKey(today)}`,
        kind: "wish-affordable",
        severity: "info",
        title: `You can afford "${cheapest.name}" right now`,
        body: `${formatCurrency(cheapest.price, currency)} \u2014 within this month's safe-to-spend.`,
        date: today,
        href: "/wishes",
        pushOnly: true,
      });
  ...
  // 8. A once-a-week nudge (Mondays) to glance back at last week's spending.
  const dow = new Date(today).getUTCDay(); // 0 = Sunday
  if (dow === 1) {
    const weekAgo = addDaysIso(today, -7);
    const lastWeekSpend = data.expenses
      .filter((e) => e.status === "paid" && e.date >= weekAgo && e.date < today)
      .reduce((sum, e) => sum + Math.abs(e.amount), 0);
    reminders.push({
      id: `weekly_summary_${today}`,
      kind: "weekly-summary",
      severity: "info",
      title: "Your week in review",
      body: `You spent ${formatCurrency(lastWeekSpend, currency)} over the last 7 days.`,
      date: today,
      href: "/analytics",
      pushOnly: true,
    });
