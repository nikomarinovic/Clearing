...
export interface AppDataContextValue {
  ...
  addWish: (wish: Omit<WishItem, "id" | "createdAt">) => WishItem;
  updateWish: (id: string, patch: Partial<WishItem>) => void;
  deleteWish: (id: string) => void;
  convertWishToExpense: (
    id: string,
    options: { removeFromWishes: boolean; status?: ExpenseStatus; date?: string },
  ) => ExpenseEntry | null;
...
export function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  // Wishes are purely aspirational — adding/editing/deleting one never
  // touches currentBalance. Only converting one to an actual expense does.
  const addWish = useCallback((wish: Omit<WishItem, "id" | "createdAt">) => {
    const full: WishItem = { ...wish, id: createId("wish"), createdAt: new Date().toISOString() };
    setData((d) => ({ ...d, wishes: [...d.wishes, full] }));
    return full;
  }, []);

  const updateWish = useCallback((id: string, patch: Partial<WishItem>) => {
    setData((d) => ({ ...d, wishes: d.wishes.map((w) => (w.id === id ? { ...w, ...patch } : w)) }));
  }, []);

  const deleteWish = useCallback((id: string) => {
    setData((d) => ({ ...d, wishes: d.wishes.filter((w) => w.id !== id) }));
  }, []);

  /** Turns a wish into a real expense (this is the only wish-related action that touches the balance), optionally removing it from the wish list. */
  const convertWishToExpense = useCallback(
    (id: string, options: { removeFromWishes: boolean; status?: ExpenseStatus; date?: string }) => {
      let created: ExpenseEntry | null = null;
      setData((d) => {
        ...
        return {
          ...
          currentBalance: round2(d.currentBalance + realizedExpenseContribution(expense)),
          wishes: options.removeFromWishes ? d.wishes.filter((w) => w.id !== id) : d.wishes,
        };
      });
      return created as ExpenseEntry | null;
    },
    [],
  );
