...
export function SavingsGoalCard({
  ...
  const shareText = `${goal.name}: ${formatCurrency(goal.currentAmount)} of ${formatCurrency(goal.targetAmount)} saved (${Math.round(progress)}%)${goal.targetDate ? ` \u2014 target ${formatDate(goal.targetDate)}` : ""}`;
    ...
    <div className="rounded-[16px] border border-[var(--border)] p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex gap-1">
          <ShareButton compact title={goal.name} text={shareText} />
