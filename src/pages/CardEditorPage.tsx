...
export default function CardEditorPage() {
  ...
  return (
    <div className="pb-6">
      ...
      <div className="sticky top-0 z-10 -mx-4 mb-5 bg-[var(--bg)]/90 px-4 pb-4 pt-1 backdrop-blur sm:static sm:mx-0 sm:bg-transparent sm:px-0 sm:pb-0 sm:pt-0 sm:backdrop-blur-none">
        <VirtualCard
          balance={data.currentBalance}
          name={data.profile.name && data.profile.name !== "there" ? data.profile.name : ""}
          currency={data.profile.currency}
          cardStyle={data.settings.cardStyle}
          cardDesign={data.settings.cardDesign}
          signatureUrl={data.profile.cardSignatureUrl}
        />
      </div>

      <Card className="mb-4">
        <h2 className="text-[15px] font-semibold text-[var(--text)]">Design</h2>
        <p className="mb-3 text-[12.5px] text-[var(--text-muted)]">The layout, texture, and how much room your signature gets.</p>
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
          {CARD_DESIGNS.map((d) => (
            <button
              key={d.id}
              type="button"
              onClick={() => updateSettings({ cardDesign: d.id })}
              aria-label={d.label}
              aria-pressed={data.settings.cardDesign === d.id}
              className={`flex flex-col items-start gap-1 rounded-[14px] p-2.5 text-left transition-colors ${
                data.settings.cardDesign === d.id ? "ring-2 ring-[var(--accent-green)]" : "ring-1 ring-[var(--border)] hover:bg-[var(--surface-2)]/60"
              }`}
            >
              <span className="flex w-full items-center justify-between text-[13px] font-semibold text-[var(--text)]">
                {d.label}
                {data.settings.cardDesign === d.id && <Check size={13} className="text-[var(--accent-green)]" />}
              </span>
              <span className="text-[11.5px] leading-snug text-[var(--text-muted)]">{d.description}</span>
            </button>
          ))}
        </div>
      </Card>
        ...
        {tab === "draw" ? (
          <div>
            <canvas
              ...
              className="h-36 w-full touch-none rounded-[14px] border border-dashed border-[var(--border-strong)] bg-[var(--surface-2)]/40"
            />
