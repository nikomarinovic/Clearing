...
import { Sun, Moon, Laptop, Download, Upload, RotateCcw, ChevronRight, Shield, Smartphone, Share, PlusSquare, CheckCircle2, Bell, BellOff, Vibrate, Zap, Wallet, ScanLine } from "lucide-react";
import { CARD_STYLES } from "../lib/cardStyles";
import { OCR_LANGUAGES } from "../lib/receiptScan";
  ...
  return (
    <div className="pb-6">
      ...
      <div className="flex flex-col gap-5">
        <p className="mb-2 mt-1 px-1 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-faint)]">Account</p>
        <Card>
        ...
        <p className="mb-2 mt-5 px-1 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-faint)]">Preferences</p>
        <Card>
        ...
        <Card>
                  ...
                  {permission === "granted"
                    ...
                    : permission === "denied"
                      ...
                      : permission === "unsupported"
                        ...
                        : "Turn on to actually receive reminders as real notifications"}
                </p>
              </div>
            </div>
          ...
          <div className="mt-3 flex items-center justify-between gap-3 rounded-[14px] border border-[var(--border)] p-3.5">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-[var(--text-faint)]">
                {permission === "granted"
                  ? "A real notification if you haven't logged anything yet today \u2014 no banner, just the notification itself."
                  : "Needs push notifications enabled above \u2014 otherwise there's nowhere for it to go."}
              </p>
            </div>
            <Toggle
        ...
        <Card>
          <div className="flex items-center gap-4">
            <div
              className="h-12 w-20 shrink-0 rounded-[10px]"
              style={{
                background: `linear-gradient(135deg, ${CARD_STYLES.find((s) => s.id === data.settings.cardStyle)?.swatch[0]}, ${CARD_STYLES.find((s) => s.id === data.settings.cardStyle)?.swatch[1]})`,
              }}
            />
            <div className="min-w-0 flex-1">
              <h2 className="text-[15px] font-semibold text-[var(--text)]">Your card</h2>
              <p className="text-[13px] text-[var(--text-muted)]">Style, signature, and how it looks on the dashboard.</p>
            </div>
            <Link
              to="/settings/card"
              className="flex shrink-0 items-center gap-1 rounded-full border border-[var(--border)] px-3.5 py-2 text-[13px] font-medium text-[var(--text)] hover:bg-[var(--surface-2)]"
            >
        ...
        <Card>
          <h2 className="mb-1 flex items-center gap-2 text-[15px] font-semibold text-[var(--text)]">
            <ScanLine size={16} /> Receipt scanning languages
          </h2>
          <p className="mb-3 text-[13px] text-[var(--text-muted)]">
            Loaded together when reading a receipt photo, so accents and totals in these languages come through correctly. Pick the ones you actually see on your receipts \u2014 loading more languages makes each scan a bit slower.
          </p>
          <div className="flex flex-wrap gap-2">
            {OCR_LANGUAGES.map((lang) => {
              const active = data.settings.ocrLanguages.includes(lang.code);
              return (
                <button
                  key={lang.code}
                  type="button"
                  ...
                  onClick={() => {
                    const next = active
                      ? data.settings.ocrLanguages.filter((c) => c !== lang.code)
                      : [...data.settings.ocrLanguages, lang.code];
                    // Always keep at least one language selected.
                    if (next.length === 0) return;
                    updateSettings({ ocrLanguages: next });
                  }}
                  className={clsx(
                    "rounded-full border px-3.5 py-1.5 text-[13px] font-medium transition-colors",
                    active
                      ? "border-[var(--accent-green)] bg-[var(--accent-green)]/12 text-[var(--accent-green)]"
        ...
        <p className="mb-2 mt-5 px-1 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-faint)]">App</p>
      ...
      {/* Second, stronger confirmation \u2014 requires typing the phrase, not just a tap, before anything is deleted.
          TODO: swap in a cancel-out animation here once it's ready (see chat). */}
      <ConfirmDialog
